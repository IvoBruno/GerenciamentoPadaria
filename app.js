...
class BakeryGame {
  ...
  reset() {
    ...
    this.history = [{
      ...
      nao_atendidos: 0,
      consumidores: 10,
      pedido: 10 // initial order in transit
    }];
  }
  ...
  processTurn(pedido) {
    ...
    // 1. Determine demand for the current week (Week 1 is 10, Week 2+ is 15)
    const demanda = this.week === 1 ? 10 : 15;
    this.lastDemanda = demanda;

    // 2. Receive materials from 'processo' into 'estoque'
    this.estoque += this.processo;

    // 3. Move 'transito' to 'processo'
    this.processo = this.transito;

    // 4. Calculate total demand (current demand + previous backorders)
    let totalDemand = demanda + this.backorders;

    // 5. Fulfill demand from stock
    let atendidos = 0;
    ...
    // 6. Calculate costs
    // Stock cost: R$ 1.00
    // Backlog/backorder cost: R$ 1.50
    const custoEstoque = this.estoque * 1.00;
    const custoFalta = this.backorders * 1.50;
    this.totalCost += (custoEstoque + custoFalta);

    // 7. Place new order (goes to transito)
    this.transito = pedido;

    // Save turn data to history
    this.history.push({
      week: this.week + 1,
  ...
  getState() {
    return {
      semana: this.week > this.maxWeeks ? this.maxWeeks : this.week,
      transito: this.transito,
      processo: this.processo,
      estoque: this.estoque,
      nao_atendidos: this.lastNaoAtendidos,
      consumidores: this.lastConsumidores,
      demanda: this.lastDemanda,
      backorders: this.backorders,
...
function setupNavigation() {
  ...
  menuNovoJogo.addEventListener("click", (e) => {
    e.preventDefault();
    if (confirm("Deseja iniciar um novo jogo? Todo o progresso atual será perdido.")) {
      resetToRegistration();
    }
  });
}
...
function setupRegistrationScreen() {
  ...
  form.addEventListener("submit", (e) => {
    ...
    document.getElementById("display-player-nick").textContent = playerInfo.nick;
...
function setupModals() {
  ...
  const nextBtns = document.querySelectorAll(".beergame-modal-conteudo-btn.-proximo");
  const prevBtns = document.querySelectorAll(".beergame-modal-conteudo-btn.-voltar");
...
function showStep(stepNumber) {
  const steps = document.querySelectorAll("#instructions-modal .beergame-modal-step");
  const stepImages = document.querySelectorAll("#instructions-modal .beergame-modal-conteudo-titulo img");
  ...
  const activeStepElements = document.querySelectorAll(`#instructions-modal .${activeStepClass}`);
...
function setupGameControls() {
  ...
  submitBtn.addEventListener("click", () => {
    ...
    if (isNaN(value) || value < 0 || value > 99) {
      alert("Por favor, insira uma quantidade de pedido válida (entre 0 e 99).");
      return;
    }
...
function updateUI(state) {
  document.getElementById("val-semana").textContent = String(state.semana).padStart(2, "0");
  ...
  document.getElementById("val-nao-atendidos").textContent = state.nao_atendidos;
...
function showGameOver(state) {
  // Fill summary stats
  document.getElementById("res-total-cost").textContent = `R$ ${state.total}`;
  
  // Calculations
  const history = state.history;
  const totalStock = history.reduce((sum, item) => sum + item.estoque, 0);
  const avgStock = (totalStock / history.length).toFixed(1);
  const totalBacklog = history.reduce((sum, item) => sum + item.nao_atendidos, 0);
